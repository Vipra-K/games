...
export default function Play() {
  ...
  async function connect(code: string, host: boolean) {
    ...
    await channel.subscribe(async status => {
      if (status !== "SUBSCRIBED") { if (status === "CHANNEL_ERROR" || status === "TIMED_OUT") setError("Could not connect to the multiplayer room."); return; }
  ...
  function startGame() {
    ...
    const now = Date.now();
    const initial: GameState = { status: "playing", startedAt: now, endsAt: now + GAME_SECONDS * 1000, lastTagAt: 0,
  ...
  function runHostLoop() {
    ...
    const tick = () => {
      ...
      const now = Date.now();
      let nextPlayers = state.players.map(p => move(p, p.id === idRef.current ? inputRef.current : (remoteInputRef.current.get(p.id) || { up:false,down:false,left:false,right:false })));
  ...
  return <main className="app-shell">
    ...
    {screen === "lobby" && <section className="panel-screen"><div className="lobby-card"><p className="eyebrow">PRIVATE ROOM</p><h2>GET READY!</h2>
      ...
      <div className="players-heading"><span>PLAYERS</span><b>{players.length}/{MAX_PLAYERS}</b></div><div className="player-list">
        {players.map((p,i)=><div className="player-row" key={p.id}><span className="avatar" style={{background:p.color}}>{p.name[0]?.toUpperCase()}</span><strong>{p.name}{p.id===idRef.current ? " (YOU)" : ""}</strong>{p.host && i===0 && <span className="crown">★</span>}</div>)}
